...
class ProductDescpMannual(CleanData):
    '''
    Method for preprocessing text in dataset, specifically arranging them in a format from which text embeddings using 
    a variety of python libraries may be readily implemented. 

    Inherits all methods from CleanData. 

    Only uses a singular dataset corresponding to product information
    '''
    def __init__(self):
    ...
    def clean_prod(self, col='product_description'):
        ...
        word_lemmitizer = WordNetLemmatizer() # Instantitaes work lemmetizer
        punct_re = re.compile(r'[^A-Za-z \n-]') # Compiles regex of all character not corresponding to alphabets, hyphens (used as part of certain words or new lines)
        self.product_frame[col] = self.product_frame[col].copy().str.replace(punct_re, '') # Strips the regex matches of the expression in previous line within product description text 
        all_text_ls = [] # Place holder for product description text for each for loop iteration (see below). Each individual list within to hold the words within each observation of the product description column 
        main_ls = [] # Empty list instantiated. To hold all the preperocessed text characters

        # Iterates through individual observations within the product description column, stripping it of stop words, lemmitizing the words, fixing the contractions, and removing any characaters not constituting alphabets
        ...
        all_text_ls = [i.lower() for i in all_text_ls] # Converts all words to lower
        self.product_frame[col] = main_ls  # Appends an additional column to the products dataframe equal to the preprecessed text within product descritpion split into a list
    ...
    def vocab_encoder(self, col='product_description', limit_it=False, vocab_limit=30000):
        ...
        Please note the vocabulary size used in the event limit_it is set to True will, in effect, be one less than vocab_size given the final encoded value is reserved for any words 
        encountered that do not exist within the vocaulary generated by the dataset
        '''
    ...
    def dataloader_preprocess(self, limit_it=True, vocab_lim=10000, context_size=5):
        '''
        Generates a dataset that may be used for the skip-gram model. 
        ...
        for idx in range(len(df)):
            prod_descript = df['product_description'].iloc[idx].split()
            init_ls.append([
                [
